...
"""
参考链接：
https://github.com/TianWen580/myscripts-openmmlab/blob/main/mmseg/folder_inference_sliding_window.py
...
def sliding_pred(img_dir, crop_size=256):
    ...
    for h in range(num_h + 1):
        ...
        for w in range(num_w + 1):
            img_crop = img[
                h * crop_size : (h + 1) * crop_size, w * crop_size : (w + 1) * crop_size
            ]
    ...
    # 在线展示
    show_result_pyplot(model, img, a_list)
